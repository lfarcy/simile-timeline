...
this.position=function(G,J){this._createHighlightDiv();
var K=Math.round(E.dateToPixelOffset(G));
var I=Math.round(E.dateToPixelOffset(J));
var H=Math.max(I-K,3);
var F=Math.max(E.getViewWidth(),4);
if(A){this._highlightDiv.style.left=K+"px";
this._highlightDiv.style.width=H+"px";
this._highlightDiv.style.height=(F-4)+"px";
}else{this._highlightDiv.style.top=K+"px";
this._highlightDiv.style.height=H+"px";
this._highlightDiv.style.width=(F-4)+"px";
}};
};
...
Timeline._Impl.prototype._autoWidthCheck=function(B){var D=this;
var C=0;
function A(){var F=D.getWidthStyle();
D._containerDiv.style[F]=C+"px";
}function E(){var H=0;
var F=D.getPixelWidth();
if(D._autoResizing){return ;
}for(var G=0;
G<D._bands.length;
G++){D._bands[G].checkAutoWidth();
H+=D._bandInfos[G].width;
}if(H>F||B){C=H;
A();
D._distributeWidths();
}}if(!D.autoWidth){return ;
}E();
};
